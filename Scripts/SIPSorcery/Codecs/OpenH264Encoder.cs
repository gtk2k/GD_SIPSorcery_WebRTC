...
namespace GD_Test3D.Codecs
{
    public class OpenH264Encoder
    {
        ...
        public OpenH264Encoder(string dllName, int width, int height, int bps, int fps, int keyframeInterval)
        {
            ...
            unsafe
            {
                EncodeFunc encodeFunc = (data, sizes, sizeCount, layerSize, frameType, frameNum) =>
                {
                ...
                encodeFuncContext = new EncodeFuncContext { Pointer = Marshal.GetFunctionPointerForDelegate(encodeFunc) };
